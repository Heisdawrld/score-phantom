...
router.get("/predict/:fixtureId", requireAuth, async (req, res) => {
  try {
    ...
    const { prediction, odds, meta, stale } = result;
...
router.get("/top-picks-today", requireAuth, async (req, res) => {
  try {
    ...
    // ── Step 2: No picks? Trigger BACKGROUND generation (non-blocking) ──────────
    // PROBLEM (user-reported): The old code awaited Promise.allSettled() on up to 15
    // getOrBuildPrediction() calls synchronously, each potentially taking 5-30s when
    // enrichment was needed (40+ BSD API calls at 4 RPS). Users waited 30-120s seeing
    // only a loading shimmer before TopPicks rendered.
    //
    // SOLUTION: Return immediately with a "warmingUp" flag so the frontend can show
    // an appropriate message + poll. Fire generation in the background (fire-and-forget).
    // The next poll (10-15s later) will pick up whatever has been generated by then.
    let warmingUp = false;
    if (rows.length === 0) {
    ...
    return res.json({
      ...
      warmingUp, // true when background generation was just triggered (frontend should poll)
      access: buildAccessPayload(req.access),
    });
  } catch (err) {
