...
router.get("/partners", adminLimiter, requireAdmin, async (req, res) => {
  try {
    ...
    const result = await db.execute(
      ...
      " (SELECT COUNT(*) FROM users WHERE partner_id = pt.id AND status = 'trial' AND CAST(trial_ends_at AS TIMESTAMP WITH TIME ZONE) > NOW()) as total_referred_trials," +
      " (SELECT COUNT(*) FROM users WHERE partner_id = pt.id AND (status = 'premium' OR (subscription_expires_at IS NOT NULL AND CAST(subscription_expires_at AS TIMESTAMP WITH TIME ZONE) > NOW()))) as total_referred_premium," +
