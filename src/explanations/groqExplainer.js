...
function buildContext(prediction) {
  ...
  // Use sanitized team names to avoid prompt injection
  const sanitizedHome = sanitizeContext(fixture.homeTeam);
  const sanitizedAway = sanitizeContext(fixture.awayTeam);
  ...
  const fmtForm = (form, teamName) => {
    ...
    return form
      ...
      .map((m) => {
        ...
        const isHome =
          hl.includes(nl.split(' ')[0]) || nl.includes(hl.split(' ')[0]);
  ...
  const oddsStr = odds
    ? `Home: ${odds.home || 'N/A'} | Draw: ${odds.draw || 'N/A'} | Away: ${
        odds.away || 'N/A'
      }`
    : 'No odds';
  ...
  const rankedMarkets = (predictions.ranked_markets || [])
    .map(
      (m, i) =>
        `  ${i + 1}. [${m.market}] ${m.pick} - Engine probability: ${m.probability}`
    )
    .join('\n');

  return `=== MATCH: ${sanitizedHome} vs ${sanitizedAway} ===
...
${sanitizedHome}: ${homeMomentum || 'N/A'}% | ${sanitizedAway}: ${
    awayMomentum || 'N/A'
  }%

RECENT FORM:
${sanitizedHome}: ${fmtForm(homeForm, sanitizedHome)}
${sanitizedAway}: ${fmtForm(awayForm, sanitizedAway)}

H2H RECORD (last meetings):
Home wins: ${
    h2h.home_win_rate !== null
      ? (h2h.home_win_rate * 100).toFixed(0) + '%'
      : 'N/A'
  } | Draws: ${
    h2h.draw_rate !== null
      ? (h2h.draw_rate * 100).toFixed(0) + '%'
      : 'N/A'
  } | Away wins: ${
    h2h.away_win_rate !== null
      ? (h2h.away_win_rate * 100).toFixed(0) + '%'
      : 'N/A'
  }
H2H avg goals per game: ${h2h.avg_total_goals || 'N/A'} | H2H BTTS rate: ${
    h2h.btts_rate !== null
      ? (h2h.btts_rate * 100).toFixed(0) + '%'
      : 'N/A'
  }

POISSON ENGINE OUTPUT:
xG Home: ${model.lambdaHome} | xG Away: ${model.lambdaAway} | Total xG: ${
    model.expectedTotalGoals
  }
Win probabilities: Home ${(predictions.match_result.home * 100).toFixed(
    1
  )}% | Draw ${(predictions.match_result.draw * 100).toFixed(
    1
  )}% | Away ${(predictions.match_result.away * 100).toFixed(1)}%
Over 2.5: ${(predictions.over_under.over_2_5 * 100).toFixed(
    1
  )}% | Under 2.5: ${(predictions.over_under.under_2_5 * 100).toFixed(1)}%
BTTS Yes: ${(predictions.btts.yes * 100).toFixed(
    1
  )}% | BTTS No: ${(predictions.btts.no * 100).toFixed(1)}%

TEAM FORM STATS:
${sanitizedHome}: Avg scored ${
    hf.avg_scored || 'N/A'
  } | Avg conceded ${hf.avg_conceded || 'N/A'} | Win rate ${
    hf.win_rate !== null
      ? (hf.win_rate * 100).toFixed(0) + '%'
      : 'N/A'
  } | Clean sheet ${
    hf.clean_sheet_rate !== null
      ? (hf.clean_sheet_rate * 100).toFixed(0) + '%'
      : 'N/A'
  }
${sanitizedAway}: Avg scored ${
    af.avg_scored || 'N/A'
  } | Avg conceded ${af.avg_conceded || 'N/A'} | Win rate ${
    af.win_rate !== null
      ? (af.win_rate * 100).toFixed(0) + '%'
      : 'N/A'
  } | Clean sheet ${
    af.clean_sheet_rate !== null
      ? (af.clean_sheet_rate * 100).toFixed(0) + '%'
      : 'N/A'
  }
