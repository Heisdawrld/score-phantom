...
if (!KEY || !SECRET) {
  throw new Error(
    'LIVESCORE_API_KEY and LIVESCORE_API_SECRET must be defined before seeding fixtures'
  );
}
  ...
  for (const f of allFixtures) {
    try {
      await db.batch([
        ...
        {
          sql:
            'INSERT OR IGNORE INTO fixtures (id, home_team_id, away_team_id, tournament_id, home_team_name, away_team_name, tournament_name, category_name, match_date, match_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          args: [
            f.match_id,
            f.home_team_id,
            f.away_team_id,
