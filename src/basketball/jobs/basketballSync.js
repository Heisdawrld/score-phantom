...
import { fetchBasketballOdds, normalizeOddsGame, extractBestBasketballMarkets } from '../services/oddsApiBasketball.js';
...
export async function syncBasketballOdds({ leagueKey = null, regions = 'us', markets = 'h2h,spreads,totals' } = {}) {
  ...
  for (const league of leagues) {
    const response = await fetchBasketballOdds(league.key, { regions, markets });
    ...
    results.push({ league: league.key, savedGames, savedMarkets, quota: response.quota });
  }

  return results;
}

export async function syncBasketballV1({ includeNbaGames = true, leagueKey = null } = {}) {
  const output = { nbaGames: null, odds: null };
  ...
  try {
    output.odds = await syncBasketballOdds({ leagueKey });
  } catch (err) {
...
export async function runBasketballPredictions({ leagueKey = null, limit = 60 } = {}) {
  await initBasketballTables();
