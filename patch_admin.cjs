...
const scriptLogic = `
    ...
    async function loadReferrals() {
      document.getElementById('standardReferralTableWrap').innerHTML = '<div class="loading">Loading...</div>';
      document.getElementById('partnerTableWrap').innerHTML = '<div class="loading">Loading...</div>';
      
      try {
        const [stdRes, partRes] = await Promise.all([
        ...
        document.getElementById('standardReferralTableWrap').innerHTML = \`<div class="error">\${err.message}</div>\`;
        document.getElementById('partnerTableWrap').innerHTML = \`<div class="error">\${err.message}</div>\`;
      }
    }
    ...
    async function settleStandardCommission(userId, email) {
      ...
      try {
        const res = await fetch('/api/admin/standard-commissions/' + userId + '/settle', {
          method: 'POST',
...
html = html.replace('// ── System Health ─────────────────────────────────────────────────────────', scriptLogic + '\n    // ── System Health ─────────────────────────────────────────────────────────');
