...
import { ChevronRight, ChevronDown, ChevronUp, Trophy, Zap, Lock, AlertCircle, Flame, BarChart2, Activity, Star, Target, Crown } from "lucide-react";
...
export function AccaSection({ isPremium }: { isPremium: boolean }) {
  ...
  const combinedOdds = Number(data?.combinedOdds) || picks.reduce((acc: number, p: any) => {
    const realOdds = Number(p.pickOdds);
    return Number.isFinite(realOdds) && realOdds > 1 ? acc * realOdds : acc;
  }, 1);
      ...
      {open && (
        <div className='px-4 pb-4 space-y-3'>
          ...
          {picks.map((pick: any, i: number) => {
            const realOdds = Number(pick.pickOdds);
            const oddsFmt = Number.isFinite(realOdds) && realOdds > 1 ? realOdds.toFixed(2) : '—';
            ...
            return (
              <div key={pick.fixtureId || i} className='flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]'>
                ...
                <div className='flex-1 min-w-0'>
                  ...
                  {/* ACCA legs use verified market prices and explicit value. */}
