        ...
        {!isLoading && !error && data && (
          <>
            <section className={cn("rounded-[30px] border p-5 overflow-hidden relative", noEdge ? "border-white/[0.06] bg-white/[0.025]" : "border-orange-300/20 bg-gradient-to-br from-orange-400/[0.12] via-white/[0.035] to-primary/[0.06]") }>
              ...
              <div className="relative flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className={cn("mt-2 text-3xl font-black", noEdge ? "text-white/55" : "text-orange-100")}>{rec.pick || "No Clear Edge"}</h2>
                  <p className="mt-2 text-xs text-white/45">{rec.market || "Basketball"} · {rec.riskLevel || "HIGH"} risk</p>
                </div>
                <div className="rounded-2xl border border-white/[0.08] bg-black/30 px-4 py-3 text-center">
              ...
              <div className="relative mt-4 space-y-2">
                {(rec.reasons || []).slice(0, 5).map((reason: string, i: number) => (
            ...
            <section className="grid grid-cols-2 gap-3">
              ...
              <Metric label="Projected Spread" value={projection.spread != null ? `${projection.spread > 0 ? (game.homeTeam || game.home_team) : (game.awayTeam || game.away_team)} by ${Math.abs(projection.spread)}` : "—"} />
            </section>
            ...
            <section className="rounded-3xl border border-white/[0.06] bg-white/[0.025] p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Metric label="Data" value={`${intel.dataQuality ?? 0}%`} />
                <Metric label="Odds" value={`${intel.oddsQuality ?? 0}%`} />
            ...
            <section className="rounded-3xl border border-white/[0.06] bg-white/[0.025] p-4">
              <div className="flex items-center gap-2 mb-3"><TrendingUp className="h-4 w-4 text-orange-300" /><h3 className="text-sm font-black uppercase tracking-widest">Other Candidates</h3></div>
              <div className="space-y-2">
                {candidates.length ? candidates.slice(0, 6).map((c: any, i: number) => (
                  <div key={i} className="flex items-center justify-between gap-3 rounded-2xl border border-white/[0.05] bg-black/20 p-3">
                    <div>
                      ...
                      <p className="text-[10px] text-white/35 uppercase tracking-widest">{c.market}</p>
                    </div>
                ...
                )) : <p className="text-xs text-white/35">No candidates available.</p>}
              </div>
            </section>
          </>
        )}
